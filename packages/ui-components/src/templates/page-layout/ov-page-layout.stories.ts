...
const meta: Meta = {
  ...
  argTypes: {
    maxWidth: { control: 'text', description: 'Override --ov-page-max-width inline' },
  },
  args: {
    maxWidth: '',
  },
...
export const Default: Story = {
  render: (args) => html`
    <ov-page-layout max-width=${args['maxWidth'] || ''}>
      ...
      <section style="padding:var(--ov-space-12) var(--ov-space-8);text-align:center">
        <ov-heading level="2">Body content is constrained to 1200 px</ov-heading>
        <ov-text variant="body" as="p" tone="secondary">
          This text and everything in the default slot sits within the centred max-width container.
          The nav and hero above stretch to the full viewport width.
        </ov-text>
      </section>
      ...
      <footer slot="footer" style="background:var(--ov-charcoal,#1e2330);color:white;padding:var(--ov-space-8);text-align:center">
        Full-width footer
      </footer>
    </ov-page-layout>
  `,
  ...
  render: () => html`
    <ov-page-layout max-width="800px">
      ...
      <section style="padding:var(--ov-space-12) var(--ov-space-8)">
        ...
        <ov-text variant="body" as="p">Useful for content-heavy pages like articles.</ov-text>
      </section>
    </ov-page-layout>
  `,
};
