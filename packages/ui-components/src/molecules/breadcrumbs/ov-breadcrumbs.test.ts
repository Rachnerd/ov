...
describe('ov-breadcrumbs', () => {
  ...
  describe('rendering', () => {
    it('renders a <nav> with aria-label="Breadcrumb"', async () => {
      const el = await fixture<OvBreadcrumbs>(html`<ov-breadcrumbs .items=${ITEMS}></ov-breadcrumbs>`);
    ...
    it('renders one <li> per item', async () => {
      const el = await fixture<OvBreadcrumbs>(html`<ov-breadcrumbs .items=${ITEMS}></ov-breadcrumbs>`);
      expect(el.shadowRoot!.querySelectorAll('li').length).to.equal(ITEMS.length);
    });
  ...
  describe('max / collapse', () => {
    ...
    it('shows all items when max is 0', async () => {
      const el = await fixture<OvBreadcrumbs>(html`<ov-breadcrumbs .items=${LONG_ITEMS}></ov-breadcrumbs>`);
      expect(el.shadowRoot!.querySelectorAll('li').length).to.equal(LONG_ITEMS.length);
    });

    it('collapses middle items when count exceeds max', async () => {
      const el = await fixture<OvBreadcrumbs>(html`<ov-breadcrumbs .items=${LONG_ITEMS} max="3"></ov-breadcrumbs>`);
  ...
  describe('accessibility', () => {
    it('passes axe', async () => {
      const el = await fixture(html`<ov-breadcrumbs .items=${ITEMS}></ov-breadcrumbs>`);
