...
describe('ov-tabs', () => {
  ...
  describe('rendering', () => {
    ...
    it('renders tab labels', async () => {
      const el = await fixture<OvTabs>(html`<ov-tabs .tabs=${TABS}></ov-tabs>`);
      const buttons = el.shadowRoot!.querySelectorAll<HTMLButtonElement>('button[role="tab"]');
    ...
    it('renders count badge next to tab with count', async () => {
      const el = await fixture<OvTabs>(html`<ov-tabs .tabs=${TABS}></ov-tabs>`);
      const activityBtn = el.shadowRoot!.querySelectorAll('button[role="tab"]')[1]!;
      expect(activityBtn.querySelector('.count')).to.exist;
      expect(activityBtn.querySelector('.count')!.textContent!.trim()).to.equal('5');
    });
  });
  ...
  describe('active state', () => {
    ...
    it('marks the active tab button as aria-selected="true"', async () => {
      const el = await fixture<OvTabs>(html`<ov-tabs .tabs=${TABS} active="activity"></ov-tabs>`);
      await elementUpdated(el);
      const buttons = el.shadowRoot!.querySelectorAll<HTMLButtonElement>('button[role="tab"]');
    ...
    it('marks inactive tabs as aria-selected="false"', async () => {
      const el = await fixture<OvTabs>(html`<ov-tabs .tabs=${TABS} active="overview"></ov-tabs>`);
      await elementUpdated(el);
      const buttons = el.shadowRoot!.querySelectorAll<HTMLButtonElement>('button[role="tab"]');
  ...
  describe('properties and attributes', () => {
    it('reflects appearance attribute', async () => {
      const el = await fixture<OvTabs>(html`<ov-tabs .tabs=${TABS} appearance="pills"></ov-tabs>`);
      expect(el.getAttribute('appearance')).to.equal('pills');
    });

    it('reflects fill attribute', async () => {
      const el = await fixture<OvTabs>(html`<ov-tabs .tabs=${TABS} fill></ov-tabs>`);
  ...
  describe('events', () => {
    it('dispatches "change" event with correct key on tab click', async () => {
      ...
      const activityBtn = el.shadowRoot!.querySelectorAll<HTMLButtonElement>('button[role="tab"]')[1]!;
      activityBtn.click();
    ...
    it('updates active property after tab click', async () => {
      const el = await fixture<OvTabs>(html`<ov-tabs .tabs=${TABS} active="overview"></ov-tabs>`);
      const settingsBtn = el.shadowRoot!.querySelectorAll<HTMLButtonElement>('button[role="tab"]')[2]!;
      settingsBtn.click();
      await elementUpdated(el);
    ...
    it('does not dispatch "change" for disabled tab', async () => {
      ...
      const archivedBtn = el.shadowRoot!.querySelectorAll<HTMLButtonElement>('button[role="tab"]')[3]!;
      archivedBtn.click();
      await elementUpdated(el);
    ...
    it('does not dispatch "change" when clicking the already-active tab', async () => {
      ...
      const overviewBtn = el.shadowRoot!.querySelectorAll<HTMLButtonElement>('button[role="tab"]')[0]!;
      overviewBtn.click();
      await elementUpdated(el);
  ...
  describe('keyboard navigation', () => {
    it('ArrowRight moves focus to the next tab', async () => {
      ...
      overviewBtn.dispatchEvent(new KeyboardEvent('keydown', { key: 'ArrowRight', bubbles: true }));
      await elementUpdated(el);
    ...
    it('ArrowLeft moves focus to the previous tab', async () => {
      ...
      activityBtn.dispatchEvent(new KeyboardEvent('keydown', { key: 'ArrowLeft', bubbles: true }));
      await elementUpdated(el);
    ...
    it('Home key moves to the first tab', async () => {
      const el = await fixture<OvTabs>(html`<ov-tabs .tabs=${TABS} active="settings"></ov-tabs>`);
      await elementUpdated(el);
      const buttons = el.shadowRoot!.querySelectorAll<HTMLButtonElement>('button[role="tab"]');
      const settingsBtn = buttons[2]!;

      settingsBtn.dispatchEvent(new KeyboardEvent('keydown', { key: 'Home', bubbles: true }));
      await elementUpdated(el);
    ...
    it('End key moves to the last enabled tab', async () => {
      ...
      overviewBtn.dispatchEvent(new KeyboardEvent('keydown', { key: 'End', bubbles: true }));
      await elementUpdated(el);
  ...
  describe('accessibility', () => {
    it('passes axe for underline appearance', async () => {
      const el = await fixture(html`<ov-tabs .tabs=${TABS} active="overview"></ov-tabs>`);
      await elementUpdated(el);
    ...
    it('passes axe for pills appearance', async () => {
      const el = await fixture(html`<ov-tabs .tabs=${TABS} appearance="pills" active="overview"></ov-tabs>`);
      await elementUpdated(el);
      await expect(el).to.be.accessible({
        ignoredRules: ['color-contrast'],
      });
