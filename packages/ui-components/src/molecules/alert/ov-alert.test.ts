...
describe('ov-alert', () => {
  ...
  describe('rendering', () => {
    it('renders body copy through the default slot', async () => {
      const el = await fixture<OvAlert>(
        html`<ov-alert>Session expiring soon.</ov-alert>`,
      );
    ...
    it('renders title from the title property', async () => {
      const el = await fixture<OvAlert>(
        html`<ov-alert title="Warning">Low storage.</ov-alert>`,
      );
    ...
    it('shows dismiss button when dismissible', async () => {
      const el = await fixture<OvAlert>(
        html`<ov-alert dismissible>Alert text.</ov-alert>`,
      );
  ...
  describe('properties and attributes', () => {
    ...
    it('reflects variant attribute', async () => {
      const el = await fixture<OvAlert>(
        html`<ov-alert variant="danger">Error.</ov-alert>`,
      );
    ...
    it('reflects dismissible attribute', async () => {
      const el = await fixture<OvAlert>(
        html`<ov-alert dismissible>Alert.</ov-alert>`,
      );
  ...
  describe('accessibility', () => {
    it('passes axe for info variant', async () => {
      const el = await fixture(
        html`<ov-alert variant="info"
          >Your session expires in 5 minutes.</ov-alert
        >`,
      );
    ...
    it('passes axe for danger variant', async () => {
      const el = await fixture(
        html`<ov-alert variant="danger" title="Payment failed"
          >Update your billing info.</ov-alert
        >`,
      );
      await expect(el).to.be.accessible({
        ignoredRules: ['color-contrast'],
    ...
    it('passes axe for dismissible alert', async () => {
      const el = await fixture(
        html`<ov-alert dismissible>New feature available.</ov-alert>`,
      );
      await expect(el).to.be.accessible({
        ignoredRules: ['color-contrast'],
      });
