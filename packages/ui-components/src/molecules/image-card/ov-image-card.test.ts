...
describe('ov-image-card', () => {
  ...
  describe('rendering', () => {
    it('renders a wrapping <a> element', async () => {
      const el = await fixture<OvImageCard>(
        html`<ov-image-card label="Amsterdam"></ov-image-card>`,
      );
    ...
    it('renders a .header bar', async () => {
      const el = await fixture<OvImageCard>(
        html`<ov-image-card label="Utrecht"></ov-image-card>`,
      );
    ...
    it('renders the label in the header', async () => {
      const el = await fixture<OvImageCard>(
        html`<ov-image-card label="Rotterdam"></ov-image-card>`,
      );
    ...
    it('sets alt="" on the photo so it is decorative (label carries the name)', async () => {
      const el = await fixture<OvImageCard>(
        html`<ov-image-card label="Groningen"></ov-image-card>`,
      );
      const img = el.shadowRoot!.querySelector<HTMLImageElement>('img.photo')!;
  ...
  describe('properties and attributes', () => {
    ...
    it('updates the label reactively', async () => {
      ...
      expect(el.shadowRoot!.querySelector('.header')!.textContent).to.include(
        'Den Haag',
      );
    });
    ...
    it('updates src reactively', async () => {
      const el = await fixture<OvImageCard>(
        html`<ov-image-card label="X" src="/a.jpg"></ov-image-card>`,
      );
      el.src = '/b.jpg';
      await elementUpdated(el);
      const img = el.shadowRoot!.querySelector<HTMLImageElement>('img.photo')!;
