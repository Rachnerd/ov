import { LitElement, html, css, svg, type TemplateResult, type SVGTemplateResult } from 'lit';
...
export class OvIcon extends LitElement {
  ...
  static override styles = [
    ...
    css`
      ...
      :host([size='sm']) { width: 12px; height: 12px; }
      :host([size='md']) { width: 16px; height: 16px; }
      :host([size='lg']) { width: 20px; height: 20px; }
      :host([size='xl']) { width: 24px; height: 24px; }

      svg { width: 100%; height: 100%; display: block; }
      ::slotted(svg) { width: 100%; height: 100%; }
    `,
  ];
...
export class OvSpinner extends LitElement {
  ...
  static override styles = [
    ...
    css`
      ...
      :host([size='lg']) { width: 32px; height: 32px; }
      :host([size='xl']) { width: 40px; height: 40px; }

      :host([tone='neutral']) { --_color: var(--color-text-secondary); }
      :host([tone='inverse']) { --_color: var(--color-text-inverse); }

      svg { width: 100%; height: 100%; animation: spin 0.9s linear infinite; }
      circle.track { stroke: var(--_color); stroke-opacity: 0.2; }
      path.head    { stroke: var(--_color); }
  ...
  protected override render(): TemplateResult {
    return html`
      <svg viewBox="0 0 24 24" fill="none" role="progressbar"
           aria-label=${this.label} aria-valuetext=${this.label}>
        <circle class="track" cx="12" cy="12" r="9" stroke-width="3"/>
        <path class="head" d="M21 12a9 9 0 0 0-9-9" stroke-width="3" stroke-linecap="round"/>
      </svg>
    `;
  }
}
