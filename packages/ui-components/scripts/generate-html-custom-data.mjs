...
const validateConfig = JSON.parse(readFileSync(resolve(root, '.htmlvalidate.json'), 'utf8'));

// The elements array is ["html5", { "ov-*": { ... }, ... }]
const customElements = validateConfig.elements.find(e => typeof e === 'object');

const tags = Object.entries(customElements).map(([name, meta]) => {
  const attributes = Object.entries(meta.attributes ?? {}).map(([attrName, attrMeta]) => {
    const entry = { name: attrName };
    if (attrMeta.description) entry.description = attrMeta.description;
    if (Array.isArray(attrMeta.enum)) {
      entry.values = attrMeta.enum.filter(v => v !== '').map(v => ({ name: v }));
    }
    return entry;
  });
...
console.log(`Generated .vscode/html.custom-data.json (${tags.length} elements)`);
